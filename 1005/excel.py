...
new_filename = 'C:\\jjy\\취약점 진단 결과.xlsx'
# 파일 이름을 C:\jjy폴더에 "취약점 진단 결과.xlsx"로 저장
...
try:
    ...
    while True:
        ...
        if line.startswith('A'):
            k = 1
            while w4.cell(row=k, column=1).value is not None:
                k = k + 1
        ...
        # w1[A'k']셀이 빈칸이 아닐때 까지 k에 1씩 더해주고 읽어들이 값을 넣는다.
            ...
            k2 = 1
            while w5.cell(row=k2, column=1).value is not None:
                k2 = k2 + 1
            w5.cell(row=k2, column=1).value = line
            continue
        elif line.startswith('C'):
            k3 = 1
            while None != w6.cell(row=k3, column=1).value:
                k3 = k3 + 1
            w6.cell(row=k3, column=1).value = line
            continue
        elif line.startswith('D'):
            k4 = 1
            while w7.cell(row=k4, column=1).value is not None:
                k4 = k4 + 1
            w7.cell(row=k4, column=1).value = line
            continue
        elif line.startswith('E'):
            k5 = 1
            while w8.cell(row=k5, column=1).value is not None:
                k5 = k5 + 1
            w8.cell(row=k5, column=1).value = line
            continue
        else:
            break
    file.close()
    print("inspect.txt 저장 성공..")
except:
    try:
...
try:
    v3 = 1
    while w7.cell(row=v3, column=1).value is not None:
        v3 = v3 + 1
    w7.merge_cells(start_row=1, start_column=1, end_row=v3 - 1, end_column=1)
    w7['A1'].value = "4. 패치 관리"
    w7['A1'].fill = PatternFill(
        fill_type='solid',
        fgColor=Color('789ABC'))
except:
    w7['A1'].value = "4. 패치 관리"
...
v4 = 1
try:
    while w8.cell(row=v4, column=1).value is not None:
        v4 = v4 + 1
    w8.merge_cells(start_row=1, start_column=1, end_row=v4 - 1, end_column=1)
    w8['A1'].value = "5. 로그 관리"
    w8['A1'].fill = PatternFill(
        fill_type='solid',
        fgColor=Color('789ABC'))
except:
    w8['A1'].value = "5. 로그 관리"
